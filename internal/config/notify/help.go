...
var (
	...
	HelpMongoDB = config.HelpKVS{
		...
		config.HelpKV{
			...
			Description: `Database name to write collections, database is auto-created`,
			Type:        "string",
		},
		config.HelpKV{
			Key:         target.MongoDBCollection,
			Description: `Collection name to store/update events, collection is auto-create`,
			Type:        "string",
		},
		config.HelpKV{
			Key:         target.MongoDBFormat,
			Description: `document layout type - 'raw' writes events as a document - 'namespace' updates the event document of a given object - 'access' adds events as a time series`,
